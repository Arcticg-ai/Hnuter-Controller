...
class HnuterController(Node):
    def __init__(self):
        ...
        self.l2 = 0.720
        self.cg_z = 0.0
        self.max_thrust_per_arm_n = 170.96
        self.tail_thrust_positive_n = 12.78
        self.tail_thrust_negative_n = 6.04
        self.tail_thrust_exponent_positive = 0.55
        self.tail_thrust_exponent_negative = 0.68
        self.motor_hover_control = 0.50
        self.motor_thrust_exponent = 0.50
        ...
        # Keep this synchronized with the persisted aircraft parameter snapshot.
        self.pitch_torque_bias = float(np.clip(
            env_float(
                'HNUTER_PITCH_BIAS',
                env_float('HNTR_PITCH_BIAS', 0.07),
            ),
            -1.0,
            1.0,
        ))
        ...
        self.servo_rate_limit_rad_s = 4.7
        self.secondary_servo_rate_limit_rad_s = 4.7
    ...
    def _tuning_snapshot(self) -> dict:
        return {
            ...
            "HNTR_CG_Z": float(self.cg_z),
            "HNTR_MAX_ARM_T": float(self.max_thrust_per_arm_n),
            "HNTR_TAIL_T_POS": float(self.tail_thrust_positive_n),
            "HNTR_TAIL_T_NEG": float(self.tail_thrust_negative_n),
            "HNTR_TAIL_EXP_P": float(self.tail_thrust_exponent_positive),
            "HNTR_TAIL_EXP_N": float(self.tail_thrust_exponent_negative),
            "HNTR_MOT_HOV": float(self.motor_hover_control),
            "HNTR_MOT_EXPO": float(self.motor_thrust_exponent),
            "HNTR_S1_RATE": float(self.servo_rate_limit_rad_s),
            "HNTR_S2_RATE": float(self.secondary_servo_rate_limit_rad_s),
    ...
    def _apply_tuning(self, data: dict):
        ...
        self.mass = max(self._tuning_float(data, 'HNTR_MASS', self.mass), 0.1)
        self.l1 = max(self._tuning_float(data, 'HNTR_L1', self.l1), 0.01)
        self.l2 = max(self._tuning_float(data, 'HNTR_L2', self.l2), 0.01)
        self.cg_z = float(np.clip(
            self._tuning_float(data, 'HNTR_CG_Z', self.cg_z), -1.0, 1.0
        ))
        self.max_thrust_per_arm_n = max(
            self._tuning_float(
                data, 'HNTR_MAX_ARM_T', self.max_thrust_per_arm_n
            ),
            1.0,
        )
        self.tail_thrust_positive_n = max(
            self._tuning_float(
                data, 'HNTR_TAIL_T_POS', self.tail_thrust_positive_n
            ),
            0.1,
        )
        self.tail_thrust_negative_n = max(
            self._tuning_float(
                data, 'HNTR_TAIL_T_NEG', self.tail_thrust_negative_n
            ),
            0.1,
        )
        self.tail_thrust_exponent_positive = float(np.clip(
            self._tuning_float(
                data, 'HNTR_TAIL_EXP_P', self.tail_thrust_exponent_positive
            ),
            0.1,
            2.0,
        ))
        self.tail_thrust_exponent_negative = float(np.clip(
            self._tuning_float(
                data, 'HNTR_TAIL_EXP_N', self.tail_thrust_exponent_negative
            ),
            0.1,
            2.0,
        ))
        self.motor_hover_control = float(np.clip(
            self._tuning_float(data, 'HNTR_MOT_HOV', self.motor_hover_control),
            0.05,
            0.95,
        ))
        self.motor_thrust_exponent = float(np.clip(
            self._tuning_float(
                data, 'HNTR_MOT_EXPO', self.motor_thrust_exponent
            ),
            0.2,
            1.5,
        ))
        self.servo_rate_limit_rad_s = float(np.clip(
            self._tuning_float(
                data, 'HNTR_S1_RATE', self.servo_rate_limit_rad_s
            ),
            0.1,
            50.0,
        ))
        self.secondary_servo_rate_limit_rad_s = float(np.clip(
            self._tuning_float(
                data,
                'HNTR_S2_RATE',
                self.secondary_servo_rate_limit_rad_s,
            ),
            0.1,
            50.0,
        ))
    ...
    def _front_thrust_to_control(self, thrust: float) -> float:
        if thrust <= 0.0:
            return 0.0
        hover_force = self.mass * self.gravity * 0.25
        control = self.motor_hover_control * math.pow(
            thrust / max(hover_force, 1e-8), self.motor_thrust_exponent
        )
        return float(np.clip(control, 0.0, 1.0))

    def _front_reachable_thrust_max_n(self) -> float:
        hover_force = self.mass * self.gravity * 0.25
        mapped_limit = hover_force * math.pow(
            1.0 / max(self.motor_hover_control, 1e-8),
            1.0 / self.motor_thrust_exponent,
        )
        return float(min(
            0.5 * self.max_thrust_per_arm_n, mapped_limit
        ))

    def _front_control_to_thrust(self, control: float) -> float:
        if not np.isfinite(control) or control <= 0.0:
            return 0.0
        hover_force = self.mass * self.gravity * 0.25
        thrust = hover_force * math.pow(
            float(control) / max(self.motor_hover_control, 1e-8),
            1.0 / self.motor_thrust_exponent,
        )
        return float(np.clip(
            thrust, 0.0, self._front_reachable_thrust_max_n()
        ))

    def _tail_thrust_to_control(self, thrust: float) -> float:
        if not np.isfinite(thrust) or abs(thrust) <= 1e-9:
            return 0.0
        if thrust > 0.0:
            control = math.pow(
                min(thrust, self.tail_thrust_positive_n)
                / self.tail_thrust_positive_n,
                self.tail_thrust_exponent_positive,
            )
            return float(np.clip(control, 0.0, 1.0))
        if not self.allow_tail_reverse:
            return 0.0
        control = math.pow(
            min(abs(thrust), self.tail_thrust_negative_n)
            / self.tail_thrust_negative_n,
            self.tail_thrust_exponent_negative,
        )
        return float(np.clip(-control, -1.0, 0.0))

    def _tail_control_to_thrust(self, control: float) -> float:
        if not np.isfinite(control) or abs(control) <= 1e-9:
            return 0.0
        if control > 0.0:
            return float(self.tail_thrust_positive_n * math.pow(
                min(float(control), 1.0),
                1.0 / self.tail_thrust_exponent_positive,
            ))
        if not self.allow_tail_reverse:
            return 0.0
        return float(-self.tail_thrust_negative_n * math.pow(
            min(abs(float(control)), 1.0),
            1.0 / self.tail_thrust_exponent_negative,
        ))
    ...
    def publish_px4_equivalent_direct_commands(self, current_time: float, dt: float):
        ...
        max_thrust_per_arm = min(
            self.max_thrust_per_arm_n,
            2.0 * self._front_reachable_thrust_max_n(),
        )
        max_tail_thrust = max(
            self.tail_thrust_positive_n, self.tail_thrust_negative_n
        )
        r_z = self.cg_z
        ...
        pitch_torque_from_horizontal_force = r_z * W[0]
        F3 = (W[4] - pitch_torque_from_horizontal_force) / self.l2
        Fz_front = W[2] - F3
        ...
        F3 = float(np.clip(
            F3,
            -self.tail_thrust_negative_n if self.allow_tail_reverse else 0.0,
            self.tail_thrust_positive_n,
        ))
        ...
        left_single = 0.5 * F1
        motor_controls = [
            self._front_thrust_to_control(right_single),
            self._front_thrust_to_control(right_single),
            self._front_thrust_to_control(left_single),
            self._front_thrust_to_control(left_single),
            self._tail_thrust_to_control(F3),
        ]
...
class HnuterWrenchModel:
    ...
    def __init__(
        ...
        tail_x_m: float = -0.720,
        reaction_torque_ratio_m: float = 0.0,
    ) -> None:
        self.configure_geometry(
            arm_half_span_m=arm_half_span_m,
            front_x_m=front_x_m,
            front_z_m=front_z_m,
            coaxial_half_separation_m=coaxial_half_separation_m,
            tail_x_m=tail_x_m,
            reaction_torque_ratio_m=reaction_torque_ratio_m,
        )

    def configure_geometry(
        self,
        arm_half_span_m: float,
        front_x_m: float,
        front_z_m: float,
        coaxial_half_separation_m: float,
        tail_x_m: float,
        reaction_torque_ratio_m: float,
    ) -> None:
...
class HnuterHardwareDRCDAController(HnuterHardwareController):
    ...
    def __init__(self) -> None:
        ...
        self._drcda_primary_rate_rad_s = max(
            0.1,
            env_float(
                'HNUTER_DRCDA_PRIMARY_RATE_RAD_S',
                self.servo_rate_limit_rad_s,
            ),
        )
        ...
        front_thrust_max = env_float(
            'HNUTER_DRCDA_FRONT_MOTOR_MAX_N',
            self._front_reachable_thrust_max_n(),
        )
        tail_thrust_positive = env_float(
            'HNUTER_DRCDA_TAIL_MOTOR_MAX_POS_N',
            self.tail_thrust_positive_n,
        )
        tail_thrust_negative = env_float(
            'HNUTER_DRCDA_TAIL_MOTOR_MAX_NEG_N',
            self.tail_thrust_negative_n,
        )
        config.thrust_max_n[:4] = front_thrust_max
        config.thrust_max_n[4] = tail_thrust_positive
        config.thrust_min_n[4] = (
            -tail_thrust_negative if self.allow_tail_reverse else 0.0
        )
        ...
        config.command_scale[8] = max(
            tail_thrust_positive, tail_thrust_negative
        )
        ...
        wrench_model = HnuterWrenchModel(
            ...
            front_x_m=env_float('HNUTER_DRCDA_FRONT_X_M', 0.0),
            front_z_m=env_float('HNUTER_DRCDA_FRONT_Z_M', self.cg_z),
            tail_x_m=-self.l2,
            reaction_torque_ratio_m=env_float(
                'HNUTER_DRCDA_REACTION_RATIO_M', 0.0
            ),
        )
        allocator_type = (
        ...
        self._drcda_tail_motor_max_positive_n = float(tail_thrust_positive)
        self._drcda_tail_motor_max_negative_n = float(tail_thrust_negative)
    ...
    def _apply_tuning(self, data: dict):
        ...
        self._drcda_primary_rate_rad_s = self.servo_rate_limit_rad_s
        self._drcda_secondary_servo_rate_rad_s = (
            self.secondary_servo_rate_limit_rad_s
        )
        self._drcda_front_motor_max_n = (
            self._front_reachable_thrust_max_n()
        )
        self._drcda_tail_motor_max_positive_n = self.tail_thrust_positive_n
        self._drcda_tail_motor_max_negative_n = self.tail_thrust_negative_n
        config.thrust_max_n[:4] = self._drcda_front_motor_max_n
        config.thrust_min_n[:4] = 0.0
        config.thrust_max_n[4] = self._drcda_tail_motor_max_positive_n
        config.thrust_min_n[4] = (
            -self._drcda_tail_motor_max_negative_n
            if self.allow_tail_reverse else 0.0
        )
        config.command_scale[ANGLE_COUNT:8] = self._drcda_front_motor_max_n
        config.command_scale[8] = max(
            self._drcda_tail_motor_max_positive_n,
            self._drcda_tail_motor_max_negative_n,
        )
        self.drcda.model.configure_geometry(
            arm_half_span_m=self.l1,
            front_x_m=0.0,
            front_z_m=self.cg_z,
            coaxial_half_separation_m=0.045,
            tail_x_m=-self.l2,
    ...
    def _preferred_drcda_command(
        ...
        return np.array([
            ...
            alpha2,
            theta2,
            self._front_control_to_thrust(controls[2]),
            self._front_control_to_thrust(controls[3]),
            self._front_control_to_thrust(controls[0]),
            self._front_control_to_thrust(controls[1]),
            self._tail_control_to_thrust(controls[4]),
        ], dtype=float)

    def _captured_drcda_state(
        self, fallback: np.ndarray
    ) -> np.ndarray:
        if not self._hardware_handover_snapshot_valid:
            return fallback.copy()
        motors = np.asarray(
            self._hardware_handover_motor_start, dtype=float
        )
        servos = np.asarray(
            self._hardware_handover_servo_start, dtype=float
        )
        if (
            motors.shape != (5,)
            or servos.shape != (4,)
            or not np.all(np.isfinite(motors))
            or not np.all(np.isfinite(servos))
        ):
            return fallback.copy()
        return np.array([
            servos[1] * self.primary_servo_angle_max_rad,
            servos[3] * self.secondary_servo_angle_max_rad
            / self.secondary_servo_gear_ratio,
            servos[0] * self.primary_servo_angle_max_rad,
            servos[2] * self.secondary_servo_angle_max_rad
            / self.secondary_servo_gear_ratio,
            self._front_control_to_thrust(motors[2]),
            self._front_control_to_thrust(motors[3]),
            self._front_control_to_thrust(motors[0]),
            self._front_control_to_thrust(motors[1]),
            self._tail_control_to_thrust(motors[4]),
        ], dtype=float)
    ...
    def publish_direct_actuator_setpoint(
        ...
        if self.drcda.last_result is None:
            initial = self._captured_drcda_state(preferred)
            self.drcda.reset(
                angle_state=initial[:ANGLE_COUNT],
                thrust_state=initial[ANGLE_COUNT:],
            )
        ...
        output_motor_controls = [
            self._front_thrust_to_control(logical_thrust[2]),
            self._front_thrust_to_control(logical_thrust[3]),
            self._front_thrust_to_control(logical_thrust[0]),
            self._front_thrust_to_control(logical_thrust[1]),
            self._tail_thrust_to_control(logical_thrust[4]),
        ]
