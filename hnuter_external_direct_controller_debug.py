...
class HnuterController(Node):
    ...
    def publish_direct_actuator_setpoint(self, motor_controls, alpha1, alpha2, theta1, theta2):
        ...
        servo_msg.control[0] = float(np.clip(alpha2 / math.radians(185.0), -1.0, 1.0))
        servo_msg.control[1] = float(np.clip(alpha1 / math.radians(185.0), -1.0, 1.0))
        servo_msg.control[2] = float(np.clip(theta2 / math.radians(180.0), -1.0, 1.0))
        servo_msg.control[3] = float(np.clip(theta1 / math.radians(180.0), -1.0, 1.0))
