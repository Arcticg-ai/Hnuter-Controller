...
class HnuterController(Node):
    def __init__(self):
        ...
        self.manual_max_position_lead_xy = 0.8
        self.manual_max_position_lead_z = 0.6
        # 稳定版控制器全程使用 PX4 position Offboard，避免起飞前后在
    ...
    def update_trajectory(self, current_time: float, dt: float):
        ...
        prev_pos = self.manual_des_pos.copy()
        manual_xy_active = abs(cmds['vx_b']) > 1e-5 or abs(cmds['vy_b']) > 1e-5
        z_target_active = abs(cmds['vz']) > 1e-5 or z_auto_vel > 1e-5
        ...
        self._limit_manual_position_lead(
            ...
            clamp_z=z_target_active,
        )
        realized_velocity = (self.manual_des_pos - prev_pos) / max(dt, 1e-3)

        self.target_position = self.manual_des_pos.copy()
        self.target_velocity = realized_velocity.astype(float)
        self.target_acceleration = np.zeros(3)
