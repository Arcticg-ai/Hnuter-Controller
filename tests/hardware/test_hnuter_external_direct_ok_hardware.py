...
from controllers.hardware.hnuter_external_direct_ok_hardware import HnuterOkHardwareController
...
class OkConfigurationTest(unittest.TestCase):
    def test_config_binds_ok_source_and_current_hardware_profile(self):
        path = (
            ...
            / 'config/hardware/hnuter_direct_ok_hardware_tuning.json'
        )
        ...
        self.assertEqual(config['direct_KR'], [5.0, 5.5, 5.0])
        self.assertEqual(config['direct_Domega'], [2.5, 2.7, 3.0])
        self.assertEqual(config['direct_tau_limit'], [2.0, 2.0, 1.5])
