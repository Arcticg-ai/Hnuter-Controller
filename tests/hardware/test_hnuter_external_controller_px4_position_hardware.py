...
from controllers.hardware import hnuter_external_controller_px4_position_hardware as hardware_module
from controllers.hardware.hnuter_external_controller_px4_position_hardware import (
    HnuterController,
    RCCommandManager,
)
...
def test_rc_manual_control_maps_to_velocity_references():
    ...
    rc.yaw_sign = -1.0
    rc.max_attitude_rate_rad_s = np.radians([40.0, 30.0])
    rc.attitude_sign = np.array([-1.0, -1.0])
    ...
    assert np.isclose(np.degrees(command['roll_rate']), -20.0)
    assert np.isclose(np.degrees(command['pitch_rate']), 7.5)
...
def test_rc_channels_fallback_recenters_throttle():
    ...
    function[RcChannels.FUNCTION_AUX_1] = 4
    function[RcChannels.FUNCTION_AUX_2] = 5
    rc.feed_rc_channels(types.SimpleNamespace(
        function=function,
        channels=[0.75, 0.0, 0.0, 0.0, 0.5, -0.25] + [0.0] * 12,
        channel_count=6,
        signal_lost=False,
    ))
    ...
    assert np.isclose(np.degrees(command['roll_rate']), -17.5)
    assert np.isclose(np.degrees(command['pitch_rate']), 8.75)
...
def test_zero_rc_command_does_not_create_automatic_climb():
    controller = types.SimpleNamespace(
        ...
        manual_des_yaw=0.2,
        manual_des_roll=0.0,
        manual_des_pitch=0.0,
        manual_attitude_limit_rad=np.deg2rad(45.0),
        min_altitude=-5.0,
        max_altitude=5.0,
    )
...
def test_aux_attitude_rates_integrate_roll_and_pitch_targets():
    controller = types.SimpleNamespace(
        _z0_initialized=True,
        _z0=0.0,
        position=np.zeros(3),
        ...
        rc_input=types.SimpleNamespace(
            get_velocity_commands=lambda dt: {
                ...
                'vz': 0.0,
                'yaw_rate': 0.0,
                'roll_rate': np.deg2rad(20.0),
                'pitch_rate': np.deg2rad(-30.0),
                'lt': 0.0,
                'rt': 0.0,
            }
        ),
    ...
    HnuterController.update_trajectory(controller, current_time=1.0, dt=0.5)

    np.testing.assert_allclose(
        np.degrees(controller.target_attitude[:2]), [10.0, -15.0]
    )
    np.testing.assert_allclose(
        np.degrees(controller.target_attitude_rate[:2]), [20.0, -30.0]
    )
